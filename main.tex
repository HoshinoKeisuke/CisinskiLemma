...
\maketitle
\cite{Cis19}  
The main objective is \Cref{prop:main}.
\begin{notation}
...
\begin{definition}
	...
	\begin{itemize}
		...
		\item %
			a small category $\dgm{I}$ that is a discrete category or either of $\omega^\op$ or $\Lambda^2_2$,
			and
		\item %
...
\end{proof}
\begin{corollary}[Lemma 5.5.6 of \cite{Cis19}]
	\label{cor:biTrivfib}
...
\end{corollary}
\begin{proof}
	$\I_\mono\boxtimesL\I_\mono$ is a cellular model for $\bisSet$ \comment{citation},
...
\begin{proof}
	...
	and let us write $q\circ j=i$ for the factorisation of $i$ with respect to this system.
	Since $j$ is in particular a left bi-anodyne extension and $i$ is a monomorphism, it suffices to show
	$q$ is a trivial fibration.
	Moreover, \Cref{cor:biTrivfib,lem:bLeftbiFibvsTrivfib} ensures that it suffices to show
	...
	By Lemma 1.3.10 of \cite{Cis19}, it suffices to show that
	the class $\{K\in\sSet\,|\,q\lollipop K\text{ is a trivial fibration }\}$ is saturated by monomorphisms.
	Since trivial fibrations are closed under products, the class is closed under coproducts.
	To show that it is closed under the pushouts and transfinite compositions indicated in 
\end{proof}
